...
import { deleteWorkAction } from '@/actions/workActions';
...
const WorksModal = (props: WorksModalProps) => {
  ...
  const deleteHandler = async () => {
    if (props.deleteSelected) {
      setLoading(true);
      const response = await deleteWorkAction(
      ...
      if (response.status === 200) {
        props.setDeleteSelected(undefined);
        alert('سابقه کار مورد نظر با موفقیت حذف شد');
      }
    }
  };
    ...
    >
      ...
      >
        <Typography variant='h6'>
          آیا مایل به حذف این سابقه کار هستید؟
        </Typography>
